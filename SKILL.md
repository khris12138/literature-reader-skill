...
2. **Extract the full text before interpreting** - This is MANDATORY:
   ...
   **For Claude Code / tools with shell access:**
   ```bash
   # Always use pdftotext first - this extracts actual text
   pdftotext "file.pdf" - | head -n 300
   ```
   
   **For other AI platforms (Kimi, DeepSeek, ChatGPT, etc.):**
   - If you have a PDF reading tool, use it to extract full text
   - If PDF extraction is not available, ask user: "请将PDF转换为文本后提供，或者复制PDF中的摘要和正文给我"
...
3. **CRITICAL VERIFICATION - Must pass before continuing:**
   ...
   ✅ **CORRECT**: You can see actual sentences from the paper:
   - The title in full
   - The complete abstract text
   - Author names and affiliations
   - Beginning of the introduction or body text
   
   **Self-check question**: Can you quote a complete sentence from the abstract?
   - If NO → You haven't read the paper yet, return to step 2
...
6. **If extraction completely fails:**
   - Report what you tried (pdftotext, Read tool, etc.)
   - Report any error messages
   - Ask user for text/Word version or manual text paste
